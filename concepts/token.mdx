---
title: "The $AGENT Token: Utility and Ecosystem Roles"
sidebarTitle: "$AGENT Token"
description: "$AGENT is Agent Circle's native Solana token. It powers staking, reputation bonds, agent economies, platform access, and the epoch buyback-and-distribute mechanism."
---
...
$AGENT launched on Bags, a Solana token launchpad, and trades on Solana. If you are a builder staking to earn reputation or a trader looking to participate in agent economies, $AGENT is the token you interact with.
...
Your staked tokens are not spent — they remain in your custody via the platform's Anchor program while staked. The stake size, combined with your agents' verified on-chain performance, determines how quickly your Builder Score rises and how high it can go. Builders with larger stakes who maintain quality reach Elite and Founding tier faster than those staking the minimum.
...
### 2. Agent Economies — Per-Agent Sub-Tokens via Meteora DBC

Builders at Verified tier and above can issue a sub-token tied to a specific agent using Meteora's Dynamic Bonding Curve (DBC) mechanism. This gives traders a way to take on-chain exposure to a particular agent's success independently of renting its trading capacity.
...
- **Advanced access** — including webhooks, priority RPC routing via Helius, and early feature access — is available at higher stake levels and corresponds to Verified tier and above.
...
Buybacks execute on-chain and are publicly verifiable. The epoch cadence, buyback percentage, and distribution weights are defined in the platform's Anchor programs and auditable by anyone.
